...
使用云计算，传统的系统边界安全措施已经不足以保护网络边界
使用零信任模式，所有访问都要经过认证和授权
- 端点安全
- 数据链路加密
...
## Hardware Security
Temper resistance: 
- how do we know the physical system has been attacked
...
tracing
差分能量分析（Differential Power Analysis, DPA）是一种基于统计分析的侧信道攻击技术，通过分析加密设备在运算过程中的能量消耗来推断密钥信息。
...
Trust Platform Module: Measured and correctness verified at every load
![](../img/Pasted%20image%2020260228064539.png)

Enclaves (Trustzone, SGX, SEV), Software Guard Extensions
- Trusted Execution Environment (TEE) 硬件沙盒环境
- **Remote attestation** (RA) is a distinct security service that allows verifier to reason about the state of untrusted remote provider (device).
在不可信 OS 上做“受保护计算”, enclave 不是“万能保险箱”，它把威胁模型从 OS 转移到固件/微码/认证链与侧信道上。
- losing visibility into malicious program ![](../img/Pasted%20image%2020260303015900.png)
- 在不可信 OS/Hypervisor 的环境下，给应用一块**隔离执行与机密性保护的 enclave**，并支持**远程证明（remote attestation）**
